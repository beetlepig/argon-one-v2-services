...
use shared_utils::load_yaml::{load_argon_config, ArgonConfig, TemperatureMatrixHashMap};
use std::thread;
...
fn main() {
    ...
    get_fan_attributes(argon_config);
}

fn get_fan_attributes(argon_config_value: ArgonConfig) {
    let interval = Duration::from_millis(argon_config_value.fan_config.interval);
    let hysteresis = argon_config_value.fan_config.hysteresis.amount;
    let only_way_down = argon_config_value.fan_config.hysteresis.only_way_down;
    let speed_temperature_matrix = argon_config_value.fan_config.matrix;
    set_fan_speed_loop(
        interval,
        hysteresis,
        only_way_down,
...
fn set_fan_speed_loop(
    ...
    hysteresis: u8,
    only_way_down: bool,
    speed_temperature_matrix: TemperatureMatrixHashMap,
) {
...
fn control_fan(
    mapped_temperature_matrix: &TemperatureMatrixHashMap,
    hysteresis: u8,
    ...
    if only_way_down && temperature_delta > 0.0 || temperature_delta.abs() > hysteresis as f32 {
        ...
        let temp_value = mapped_temperature_matrix.get(&rounded_device_temp);
